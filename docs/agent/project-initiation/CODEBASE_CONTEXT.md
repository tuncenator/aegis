...
>
> Last updated by: Checkpoint 8 - consolidated Phase 9 updates (FINAL)
...
| `README.md` | Full project README (replaced Phase 1 skeleton in Phase 9) | 67 lines, ASCII only, pipeline diagram, install/config/toggles/dev/spec sections |
...
### Created in Phase 9 (install + readme)

| File Path | Purpose | Notes |
|-----------|---------|-------|
| `install.sh` | Idempotent bash installer: symlinks plugin into `$PLUGIN_BASE/aegis`, optionally refreshes rule snapshot, symlinks `bin/aegis` to `~/.local/bin/aegis`, writes starter TOML to `~/.config/aegis/aegis.toml` | Executable, `set -e`, `set -u`. Every step has skip-if-present branch. Prefers `~/Sync/.claude/plugins` over `~/.claude/plugins`. |
| `README.md` | Full project README with pipeline diagram, install, config, toggles, standalone mode, dev commands, spec link | Replaced Phase 1 5-line skeleton. ASCII only, `->` arrows. |
