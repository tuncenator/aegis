...
def test_status_no_session_returns_message(isolated_state):
    r = _run("status")
    assert r.returncode == 0
    assert "no session" in r.stdout.lower() or "no active" in r.stdout.lower()


def test_off_then_on_round_trip(isolated_state):
    r = _run("off", "--session", "s1", env={**os.environ, "AEGIS_STATE_DIR": str(isolated_state)})
    assert r.returncode == 0
    ...
    r = _run("on", "--session", "s1", env={**os.environ, "AEGIS_STATE_DIR": str(isolated_state)})
    assert r.returncode == 0
    obj = json.loads(state_file.read_text())
...
def test_status_specific_session(isolated_state):
    ...
    r = _run("status", "--session", "s2",
             env={**os.environ, "AEGIS_STATE_DIR": str(isolated_state)})
    assert r.returncode == 0
